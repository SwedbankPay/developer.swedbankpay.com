...
`{{ transaction }}` transaction made against a {{ include.api_resource }} payment. You can
...

{
    ...
    "{{ plural }}": { {% if include.api_resource  == "invoice" %}
        "receiptReference": "AH12355", {% endif %}
        "id": "/psp/{{ include.api_resource  }}/payments/{{ page.payment_id }}/{{ plural }}",
        "{{ transaction }}List": [{
            "id": "/psp/{{ include.api_resource  }}/payments/{{ page.payment_id }}/{{ plural }}/{{ page.transaction_id }}",{% if include.api_resource == "swish" %}
            "swishPaymentReference": "8D0A30A7804E40479F88FFBA26111F04",
            "swishStatus": "PAID",{% endif %}{% if transaction == "authorization" %}
            "consumer": {
                    "id": "/psp/{{ include.api_resource }}/payments/{{ page.payment_id }}/consumer"
                },
                "legalAddress": {
                    "id": "/psp/{{ include.api_resource }}/payments/{{ page.payment_id }}/legaladdress"
                },
                "billingAddress": {
                    "id": "/psp/{{ include.api_resource }}/payments/{{ page.payment_id }}/billingaddress"
                },{% endif %}
            "transaction": {
                ...
                "operations": [{% if transaction == "authorization" %}
                       {
                            "method": "POST",
                            "href": "{{ page.api_url }}/psp/{{ include.api_resource }}/payments/{{ page.payment_id }}/authorizations",
...
| └─➔&nbsp;`payeeReference`         | `string`  | {% include field-description-payee-reference.md documentation_section=include.documentation_section describe_receipt=true %}                                                                                         | {% if include.api_resource == "invoice" %}
