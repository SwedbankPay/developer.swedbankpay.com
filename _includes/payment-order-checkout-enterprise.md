...
| | └➔&nbsp;`requestDeliveryInfo`                       | `bool` | Set to `true` if you want Swedbank Pay to return delivery information from the payment instruments which supports this. The delivery information will then be displayed as the `shippingAddress`. |
