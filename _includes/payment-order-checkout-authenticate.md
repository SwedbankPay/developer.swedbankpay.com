...
| └➔&nbsp;`guestMode`       | `bool`     | Set to `true` or `false`. Indicates if the payer chose to pay as a guest or not. When using the Authenticated implementation,this is triggered if the payer chooses to proceed as a guest when given the option during checkin.                                                                                                                                                   |
