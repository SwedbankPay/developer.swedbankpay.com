...
{% if documentation_section contains "pay/enterprise" %} If you want to
...
here.](/pay/enterprise/redirect#payment-order-request) {% endif %} {% if
documentation_section contains "pay/payments-only" %} [click
here.](/pay/payments-only/redirect#payment-order-request) {% endif %}
...
| | {% f digitalProducts %}                       | `bool` | Set to `true` for merchants who only sell digital goods and only require `email` and/or `msisdn` as shipping details. Set to `false` if the merchant also sells physical goods. | {% if documentation_section contains "pay/enterprise" %}
