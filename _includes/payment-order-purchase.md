...
| {% f instrument %}     | `string`     | The payment method used. Selected by using the [3-D Secure 2]({{ features_url }}/optional/instrument-mode).                                                                                                           |
| {% f paymentToken %}   | `string`     | The payment token created if `generatePaymentToken: true` was used. Enables future [one-click payments]({{ features_url }}/optional/one-click-payments) – with the same token.                                                                 |
