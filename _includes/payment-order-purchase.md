...

{
    "paymentorder": {
        ...
        "payer": {  {% if include.documentation_section == "checkout" %}
...
| {% icon check %} | └➔&nbsp;`instrument`              | `string`     | The payment instrument used. Selected by using the [Instrument Mode]({{ other_features_url }}#instrument-mode).                                                                                                                                                                                          |
...
| {% icon check %} | └─➔&nbsp;`logoUrl`      | `string`     | If the configuration is activated and you send in a `logoUrl`, then the Swedbank pay logo is replaced with the logo sent in. If you do not send in a logoUrl, then no logo and no text is shown. If the configuration is deactivated, and you send in a logoUrl, this is not handled.                                                                                                                                                                                  |
...
|                  | └➔&nbsp;`payer`                   | `object`     | The `payer` object containing information about the payer relevant for the payment order.                                                                                                                                                                                                                | {% if include.documentation_section == "checkout" %}     |
...
|                  | └─➔&nbsp;`email`                  | `string`     | The e-mail address of the payer. Will be used to prefill the Checkin as well as on the payer's profile, if not already set. Increases the chance for [frictionless 3-D Secure 2 flow]({{ other_features_url }}#3-d-secure-2).                                                                            |
|                  | └─➔&nbsp;`msisdn`                 | `string`     | The mobile phone number of the Payer. Will be prefilled on Checkin page and used on the payer's profile, if not already set. The mobile number must have a country code prefix and be 8 to 15 digits in length. The field is related to [3-D Secure 2]({{ other_features_url }}#3-d-secure-2).           |
|                  | └─➔&nbsp;`workPhoneNumber`        | `string`     | The work phone number of the payer. Optional (increased chance for frictionless flow if set) and is related to [3-D Secure 2]({{ other_features_url }}#3-d-secure-2).                                                                                                                                    |
|                  | └─➔&nbsp;`homePhoneNumber`        | `string`     | The home phone number of the payer. Optional (increased chance for frictionless flow if set) and is related to [3-D Secure 2]({{ other_features_url }}#3-d-secure-2).                                                                                                                                    |
...
| {% icon check %} | └─➔&nbsp;`amount`                 | `integer`    | {% include field-description-amount.md %}                                                                                                                                                                                                                                                                |
| {% icon check %} | └─➔&nbsp;`vatAmount`              | `integer`    | {% include field-description-vatamount.md %}                                                                                                                                                                                                                                                             |
{% include risk-indicator-table.md documentation_section=include.documentation_section %}
...
| └➔&nbsp;`instrument`     | `string`     | The payment instrument used. Selected by using the [Instrument Mode]({{ other_features_url }}#instrument-mode).                                                                                                           | {% if include.documentation_section == "payment-menu" %} |
