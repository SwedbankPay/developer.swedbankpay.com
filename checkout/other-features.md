...
|                   | └➔&nbsp;`no3DSecureForStoredCards` | `boolean`    | `true` if 3DSecure are to be disabled for this payment if a stored card is used; otherwise `false` per default. To use this feature it has to be enabled on the contract with Swedbank Pay.                  |
|                   | └➔&nbsp;`noCvcForStoredCards`      | `boolean`    | `true` if the CVC field can be disabled for this payment is used; otherwise `false` per default. To use this feature it has to be enabled on the contract with Swedbank Pay.                                 |
...
|      ✔︎︎︎︎︎       | `class`               | `string`  | The classification of the order item. Can be used for assigning the order item to a specific product category, for instance. Swedbank Pay use this field for statistics, for example `MobilePhone`.   |
|                   | `itemUrl`             | `string`  | The URL to a page that can display the purchased item for the payer.                                                                                                                                  |
