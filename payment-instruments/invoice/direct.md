...
| {% icon check %}︎︎︎︎︎ | {% f operation %}               | `string`      | The operation that the `payment` is supposed to perform. The [`FinancingConsumer`][financing-consumer] operation is used in our example. |
| {% icon check %}︎︎︎︎︎ | {% f intent %}                  | `string`      | `Authorization` is the only intent option for invoice. Reserves the amount, and is followed by a [cancellation][cancel] or [capture][capture] of funds.                                                                                                                                                                |
...
| {% icon check %}︎︎︎︎︎ | {% f type, 2 %}                   | `string`      | Use the `Invoice` type here                                                                                                                                                                                                                                                                                            |
